...
const categoryData = ['All', 'Festivals', 'Concerts', 'Club Nights', 'Sports', 'Theatre & Comedy', 'vouchers & Days out'];
		...
		<>
			<Box direction="row" className="filterBoxBorder" pad="none">
				<DropButton
					...
					icon={
						<Box width="2.5rem" pad="small" round="medium" background="status-error">
					...
					dropContent={
						<Box pad="medium">
							{categoryData.map(element => (
								<Button
									className={category === element ? 'selectButton dropDownButton' : 'dropDownButton'}
									plain
									label={element}
